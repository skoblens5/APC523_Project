...
class DropSimulation:
    ...
    def step(self, delta, b, xi, dt):
        ...
        r_factor = delta**3 / b**2
        derivatives = np.gradient(delta, xi, edge_order=2)
        convective = 3 * delta**2 / b**2 * derivatives**2
        delta_new[1:-1] = delta[1:-1] + dt * self.G / (3 * self.NU) * (r_factor[1:-1] * derivatives[1:-1] + convective[1:-1])
    ...
    def step(self, delta, b, xi, dt):
        ...
        for i in range(1, self.N - 1):
            ...
            delta_new[i] = delta[i] + dt * self.G / (3 * self.NU) * (r_factor * derivative + convective)
        delta_new[0] = delta_new[1]  # Enforce boundary conditions
        delta_new[-1] = 0
        return delta_new
    """
    ...
    def update_profile(self, delta, b, xi, dt):
        """
        Updates the drop profile using either an explicit or implicit method based on the specified method.
        ...
        if self.method == 'explicit':
            ...
            A_reduced = A[1:-2, 1:-2]  # Reduced matrix for stability calculation
            ...
            delta_new = step(self.delta, self.b, self.xi, dt)  # Calculate the new profile using the updated dt
    ...
    def volume_shooting(self, b, delta_new):
        ...
        while err > 1E-14:
            ...
            V_new = self.calculate_volume(b_guess*self.xi, delta_new)
    ...
    def run_simulation(self):
        ...
        while time_val < self.SIMULATION_TIME:
            ...
            if self.method == 'implicit':
                delta_new = update_profile(self.delta, self.b, self.xi, self.V0, dt)
                b = volume_shooting(self.b, delta_new)
                ...
                delta_new = self.step(self.delta, self.b, self.xi, dt)
                V_new = self.calculate_volume(self.b, delta_new)
